...
const gameImage = document.querySelector('.hangman'); //hide the game options section initially
...
const matchTrue = function (letter, word) {
  ...
  let interimGuessWord = word.replace(letter, '-'); //This is where we want to update the guess word on the web page.
...
const guessMatch = function (gWord) {
  ...
  if (guessCount > 0) {
    let guessLetter = inputLetter.value.toLowerCase(); //reset the input box

    inputLetter.value = '';
    labelLettersPicked.textContent += guessLetter.toUpperCase() + ' , '; //increment the number of tries

    tries = tries + 1;
    labelNumTries.textContent = tries; //is it the initial try?
    ...
    if (guessCount == 10 && tries == 1) {
      guessWord = gameWord; //get the length of the guess word.

      gameWordLength = guessWord.length; //set the solved word.
    ...
    if (guessWord.includes(guessLetter)) {
      ...
      keepChecking();
      inputLetter.focus(); //decide if the entire word has been guessed.  If they have let the know and then ask them if they want to play again.
      ...
      labelStatus.textContent = "Bad guess! Try again!"; //update the image to show the next one in the series.  If it's the first bad guess, unhide the image.
      ...
      inputLetter.focus(); //This is where I need to check to see if the turns left is 0.  If so, tell you user, they lost.
...
btnStartGame.addEventListener('click', function (e) {
  e.preventDefault(); //validate that the user has entered in only letters
